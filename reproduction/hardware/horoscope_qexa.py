...
def fold_garbage_hw(
    ...
    Mirrors the gate-level from-left structure of :func:`fold_circuit` so the
    total gate count matches genuine folding exactly.  For every fold copy:

      * two-qubit gates (e.g. ``cz``) are re-applied **unchanged on the same
        qubits**, keeping the two-qubit gate count and the coupling map
        identical to genuine folding (the dominant hardware-noise channel);
      * single-qubit gates are replaced by a **random** single-qubit rotation,
        so the inserted block no longer implements ``G^dagger G = I``.

    The result therefore has the same gate-type distribution (and hence
    approximately the same residual error rate) as genuine folding, but the
    coherent signal at ``lambda > 1`` is destroyed by construction.
    """
    if scale_factor <= 1.0:
        return circuit.copy()
    ...
    for i, (op, qubits, clbits) in enumerate(ops):
        ...
        for _ in range(k):
            for _ in range(2):  # two garbage gates replacing (G^dagger, G)
                if op.num_qubits >= 2:
                    # keep the two-qubit gate on its original (connected) qubits
                    folded.append(op, qubits, clbits)
                else:
                    folded.rz(float(rng.uniform(0.0, 2.0 * np.pi)), qubits[0])
    return folded
...
def get_qexa_backend(backend_name: str = BACKEND_NAME):
    """Connect to the IQM Euro-Q-Exa backend through the MQSS adapter."""
    token = _read_mqss_token()
...
def prepare_circuits(backend=None, scale_factors=SCALE_FACTORS) -> dict:
    ...
    n2q = sum(v for k, v in qc_t.count_ops().items() if k in ("cz", "cx", "ecr"))
    log.info(f"Transpiled TC1: {qc_t.size()} gates, {n2q} two-qubit, depth {qc_t.depth()}")
